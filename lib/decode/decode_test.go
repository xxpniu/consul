...
func TestHookNormalizeHCLNestedBlocks_DoesNotModifySliceTargets(t *testing.T) {
	source := `
slice {
    name = "first"
}
...
func decodeHCLToMapStructure(source string, target interface{}) error {
	...
	md := new(mapstructure.Metadata)
	decoder, err := mapstructure.NewDecoder(&mapstructure.DecoderConfig{
		DecodeHook: HookNormalizeHCLNestedBlocks,
		Metadata:   md,
		Result:     target,
	})
...
func TestHookNormalizeHCLNestedBlocks_ErrorsWithMultipleNestedBlocks(t *testing.T) {
	source := `
item {
    name = "first"
}
...
func TestHookNormalizeHCLNestedBlocks_UnpacksNestedBlocks(t *testing.T) {
	source := `
item {
    name = "first"
}
